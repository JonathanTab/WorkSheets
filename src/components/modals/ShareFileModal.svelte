<script>
    ...
    $effect(() => {
        storage.users.list().then(u => { allUsers = u; }).catch(() => {});
    ...
    let filteredUsers = $derived.by(() => {
        ...
        const existing = new Set(currentFile.sharedWith?.map(s => s.username) ?? []);
    ...
    async function addShare() {
        if (!addUsername.trim()) { addError = "Enter a username"; return; }
        addError = "";
        saving = true;
        try {
            const perms = addPerm === "write" ? ["read", "write"] : ["read"];
            const updated = await storage.drive.shareFile(currentFile.id, addUsername.trim(), perms);
            currentFile = { ...updated };
            addUsername = "";
            userSearch = "";
        } catch (e) {
    ...
    async function revokeShare(username) {
        saving = true;
        try {
            const updated = await storage.drive.revokeFile(currentFile.id, username);
            currentFile = { ...updated };
        } catch (e) {
    ...
    async function togglePublicRead() {
        saving = true;
        try {
            const updated = await storage.drive.setFilePublic(currentFile.id, !currentFile.publicRead, currentFile.publicWrite);
            currentFile = { ...updated };
        } finally {
            saving = false;
        }
...
<div class="share-modal">
    ...
    {#if currentFile.sharedWith?.length > 0}
        <div class="section">
            ...
            {#each currentFile.sharedWith as share (share.username)}
                <div class="share-row">
                    <div class="avatar sm">{share.username[0].toUpperCase()}</div>
                    <span class="share-user">{share.username}</span>
                    <span class="share-perm">{permLabel(share)}</span>
                    <button class="revoke-btn" title="Remove" onclick={() => revokeShare(share.username)} disabled={saving}>
                        {@html trashSvg}
                    </button>
                </div>
            {/each}
        ...
        <div class="add-row">
            <div class="user-input-wrap">
                <input
                    ...
                    oninput={() => { addUsername = userSearch; addError = ""; }}
                />
                {#if userSearch && filteredUsers.length > 0}
                    <div class="user-dropdown">
                        {#each filteredUsers.slice(0, 6) as u (u.username)}
                            <button class="user-option" onclick={() => selectUser(u.username)}>
                                <div class="avatar sm">{u.username[0].toUpperCase()}</div>
                                <div>
                                    <div class="opt-name">{u.displayName || u.username}</div>
                                    {#if u.displayName}<div class="opt-sub">@{u.username}</div>{/if}
                                </div>
                            </button>
                        {/each}
                    </div>
    ...
    <div class="section">
        ...
        <label class="toggle-row">
            <input type="checkbox" checked={currentFile.publicRead} onchange={togglePublicRead} disabled={saving} />
            <span>Anyone with the link can view</span>
        </label>
        {#if currentFile.publicRead}
            <label class="toggle-row">
                <input type="checkbox" checked={currentFile.publicWrite} onchange={togglePublicWrite} disabled={saving} />
                <span>Anyone with the link can edit</span>
            </label>
            <Button variant="secondary" size="sm" icon={linkSvg} iconPosition="left" onclick={copyLink}>
                Copy link
            </Button>
        {/if}
    </div>
...
<style>
    ...
    .owner-name { font-size: 0.875rem; font-weight: 500; color: var(--color-text); }
    .owner-badge {
        font-size: 0.7rem;
        font-weight: 600;
    ...
    .share-user { flex: 1; font-size: 0.875rem; color: var(--color-text); }
    .share-perm { font-size: 0.8125rem; color: var(--color-text-muted); }
    ...
    .revoke-btn :global(svg) { width: 13px; height: 13px; }
    .revoke-btn:hover { background: var(--color-fill); color: #ef4444; }
    .revoke-btn:disabled { opacity: 0.4; pointer-events: none; }
    ...
    .user-input-wrap { position: relative; flex: 1; }
    ...
    .user-option:hover { background: var(--color-fill); }

    .opt-name { font-size: 0.875rem; color: var(--color-text); font-weight: 500; }
    .opt-sub  { font-size: 0.75rem; color: var(--color-text-muted); }
