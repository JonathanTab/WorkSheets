    ...
    function refresh() {
        ...
        for (const [clientId, state] of states) {
            ...
            next.push({
                ...
                isSelf: name === currentUser,
            });
        }
        // Sort: self first, then alphabetical
        next.sort((a, b) => (b.isSelf ? 1 : 0) - (a.isSelf ? 1 : 0) || a.name.localeCompare(b.name));
        users = next;
    }
...
{#if users.length > 0}
<div class="flex items-center gap-1" role="group" aria-label="Active collaborators">
